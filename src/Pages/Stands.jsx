...
import '../Components/StandsComponent/StandsComponent.css';
...
function Stands(){
  ...
  useEffect(() => {
    ...
    fetchStores();
    //cambio temporal se borro shelfs para evitar generacion masiva de datos
  }, [currentPage]);
    ...
    return (
      ...
      <section className='standsContainer' id={element.storage.id_almacen}>
        <DefaultStand event={() => { setModalStandOpen(true) }} />
        {
          shelfs.map(shelf => {
            return shelf.map(s => {
              // Aqui habia un error cambie la forma en como lo validas
              if (s.id_almacen === element.storage.id_almacen) { // Usar ID en lugar de nombres
